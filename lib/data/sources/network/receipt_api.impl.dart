...
class ReceiptApiImpl implements ReceiptApi {
  ...
  Future<ReceiptDto> sendReceipt(ReceiptDto receipt) async {
    try {
      ...
      return receipt.copyWith(
          ...
          isSynced: true,
          receiptSeq:
              (receipt.receiptSeq == '0' || receipt.receiptSeq == null || (receipt.receiptSeq?.isEmpty ?? false))
                  ? response['id'].toString()
                  : receipt.receiptSeq,
          receiptDateTime: (receipt.receiptDateTime == null || (receipt.receiptDateTime?.isEmpty ?? false))
  ...
  Map<String, dynamic> toPosJson(ReceiptDto receipt) => {
        ...
        'datetime': FormatterService().datePosFormatter(receipt.receiptDateTime),
  ...
  Future<ReceiptDto> getReceipt(ReceiptDto receipt) async {
    try {
      ...
      return receipt.copyWith(
          ...
          params: receipt.params.copyWith(receivedDollar: (response['received_dollar'] ?? 0).toInt()));
    } catch (e) {
      if (e is DioException) {
        throw ServerException(e.response?.data.toString() ?? 'Server Error', e.response?.statusCode ?? 500);
      }
          ...
          try {
            return PaginatedDto.fromJson(
              ...
              (json) => ReceiptDto(
                  ...
                  receiptDateTime: FormatterService().datePosParser(json['datetime']),
                  ...
                  payments: List.from(json['payments']).map((e) => PaymentsDto.fromJson(e)).toList(),
  ...
  @override
  Future<List<ReserveDto>> getReserves() async => _dioClient.getRequest(
        NetworkConstants.reserve,
        converter: (response) => (response['results'] as List).map((e) => ReserveDto.fromJson(e)).toList(),
      );

  @override
  Future<void> realizationReserve(int reserveId) async {
    try {
      await _dioClient.postRequest('${NetworkConstants.reserve}/$reserveId/realization');
    } catch (e) {
      if (e is DioException) {
  ...
  @override
  Future<void> downloadInvoice(int receiptId, int companyId, int contractId) async {
    try {
  ...
  Future<ZReportInfoDto> getReportInfo() async {
    try {
      ...
      final response = await _dioClient.getRequest(NetworkConstants.posReportInfo, queryParameters: {'date': date});
  ...
  Future<CalculateKVIDiscountResponse> getDiscount(CalculateKVIDiscountRequest data) async {
    try {
      final res = await _dioClient.postRequest(
        ...
        converter: (response) => CalculateKVIDiscountResponse.fromJson(response),
      );
